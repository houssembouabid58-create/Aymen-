...
(function(){
  ...
  if(bookingForm){
    ...
    bookingForm.addEventListener('submit', (e) => {
      ...
      // 2) Open the email client with the same message (slight delay avoids popup conflicts)
      setTimeout(() => {
        window.location.href = `mailto:${OWNER_EMAIL}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(msg)}`;
      }, 400);

      showStatus('ok',
        'تم فتح واتساب والبريد الإلكتروني برسالتكم — أكملوا الإرسال من هناك وسنتواصل معكم قريباً.',
        'WhatsApp et votre messagerie se sont ouverts avec votre message — finalisez l\'envoi, nous vous contacterons bientôt.');
    });
  }
})();
